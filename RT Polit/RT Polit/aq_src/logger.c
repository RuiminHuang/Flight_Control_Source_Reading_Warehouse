...
void loggerSetup(void) {
	...
	switch (loggerFields[i].fieldId) {
		...
		loggerData.fp[i].fieldPointer = 0;//(void *)&gimbalData.triggerLogVal;
		break;
	    case LOG_ACC_BIAS_X:
		...
		loggerData.fp[i].fieldPointer = 0;//(void *)&canSensorsData.values[CAN_SENSORS_PDB_BATA];
		break;
	    case LOG_CURRENT_EXT:
		loggerData.fp[i].fieldPointer = (void *)&analogData.extAmp;
		break;
	    case LOG_VIN_PDB:
		loggerData.fp[i].fieldPointer =0;// (void *)&canSensorsData.values[CAN_SENSORS_PDB_BATV];
		break;
	}
