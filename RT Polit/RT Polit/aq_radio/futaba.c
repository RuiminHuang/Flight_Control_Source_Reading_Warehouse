...
void futabaInit(radioInstance_t *r, char *serial_name) {
    memset((void *)&futabaData, 0, sizeof(futabaData));

    r->serialPort = serialOpen(serial_name, FUTABA_BAUD, 0, FUTABA_RXBUF_SIZE, 0);
	
    serialChangeParity(r->serialPort, PARITY_EVEN);
    serialChangeStopBits(r->serialPort, STOP_BITS_2);
}
